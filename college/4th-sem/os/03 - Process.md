...
**Exam Probability:** Medium (2–5 Marks)
...
**An Interrupt is a signal sent to the CPU that temporarily stops the current execution and requests the CPU to perform another important task.**
...
You stop studying and answer the phone.
...
This is exactly how an interrupt works.
...
|---|---|
|Generated by hardware devices|Generated by programs|
|External to CPU|Internal to CPU|
|Example: Keyboard|Example: System Call|
...
**An Interrupt is a signal that temporarily stops the current execution of a process and requests the CPU to perform another task. After handling the interrupt, the CPU resumes the previous process. Interrupts are mainly of two types: Hardware Interrupts and Software Interrupts.**
...
Usually asked with Priority Scheduling.
...
**Priority is a value assigned to a process that determines its importance for CPU allocation.**

A process with higher priority gets CPU before lower-priority processes.
...
A newly arrived higher-priority process can interrupt the running process.
...
Once a process gets CPU, it runs until completion.
...
1. Important processes get CPU first.
    
2. Suitable for real-time systems.
    
3. Faster execution of critical tasks.
...
**Priority is a value assigned to a process indicating its importance. In Priority Scheduling, the CPU is allocated to the process with the highest priority. It can be implemented using either preemptive or non-preemptive scheduling.**
...
# 11. SCHEDULING OBJECTIVES ⭐⭐⭐

**Exam Probability:** Medium

Usually asked as a short note.
...
**Scheduling Objectives are the goals that a CPU scheduling algorithm tries to achieve for efficient system performance.**

---

# Main Scheduling Objectives

## 1. Maximize CPU Utilization

CPU should remain busy most of the time.
...
More processes should be completed in less time.

```text
More Completed Processes
...
Processes should spend less time in the ready queue.

---

## 5. Minimize Response Time

Users should receive quick responses.
...
**Scheduling Objectives are the goals of CPU scheduling algorithms. The main objectives are maximizing CPU utilization and throughput while minimizing turnaround time, waiting time, and response time. Scheduling should also ensure fairness among processes.**
...
This single shortcut can reproduce the entire Scheduling Objectives answer in the exam.